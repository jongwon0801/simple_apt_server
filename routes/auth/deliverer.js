    ...
    connection.query(
      ...
      (err) => {
        ...
        connection.query(smsQuery, [hp, "15774594", smsMsg], (err2) => {
          connection.release();
          if (err2) return res.status(500).json({ type: "error", message: err2 });
...
router.post("/registerDeliverer", (req, res) => {
  ...
  if (!hp || !authNum) return res.status(400).json({ type: "error", message: "hp/authNum 필수" });
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(
      ...
      [hp, authNum],
      (err, results) => {
        if (err) { connection.release(); return res.status(500).json({ type: "error", message: err }); }
        if (!results.length) { connection.release(); return res.status(401).json({ type: "error", message: "인증번호 불일치" }); }
...
router.get("/checkDeliverer/:hp", (req, res) => {
  ...
  pool.query("SELECT staffSq, hp, regDate FROM deliveryStaff WHERE hp = ?", [hp], (err, results) => {
    if (err) return res.status(500).json({ message: err.message });
    if (!results.length) return res.json({ exists: false, message: "등록되지 않은 번호입니다." });
