...
// 테이블에 인증번호 유니크 제약 추가
router.get(
  "/authSmsLocker/:yid/:jumper/:serial/:senderHp/:receiverHp",
  (req, res) => {
    const { yid, jumper, serial, senderHp, receiverHp } = req.params;
    if (!senderHp || !receiverHp)
      return res.status(400).json({ message: "송신/수신자 번호 필요" });

    const authNum = randomString(5, "N");
    const smsMsg = `무인 택배함 ${label} 에 물품 보관, 인증번호[${authNum}]`;
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
       ...
       WHERE yid = ? AND jumper = ? AND serial = ?`,
        [authNum, senderHp, receiverHp, yid, jumper, serial],
        (err, result) => {
          if (err) {
            connection.release();
...
router.get("/verifyLockerAuth/:authNum", (req, res) => {
  ...
  pool.query(
    `SELECT yid, jumper, serial, label 
       FROM locker 
       WHERE authNum=?`,
    [authNum],
    (err, results) => {
